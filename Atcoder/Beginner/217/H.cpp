...
{
    scanf("%d",&n);
    for(int i=1;i<=n;i++) scanf("%d%d%d",&t[i],&d[i],&x[i]);
    SlopeTrick<ll> st;
    for(int i=0;i<n;i++) st.add_abs(0);
    int cur=0;
    for(int i=1;i<=n;i++)
    {
        int dif=t[i]-cur;
        st.shift(-dif,dif);
        if(d[i]==0) st.add_a_minus_x(x[i]);
        else st.add_x_minus_a(x[i]);
        cur=t[i];
    }
    printf("%lld\n",st.min_f);
    return 0;
