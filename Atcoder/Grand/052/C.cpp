...
int main()
{
    ...
    for(int i=0;i<=n;i++)
    {
        int sum=n-i-p;
        if(sum<0) continue;
        for(int j=0;j*(p-2)<=sum&&j<=i;j++)
        {
            int nsum=sum-j*(p-2);
            if(j&1) 
            {
                dec(res,1LL*comb(n,i)*comb(i,j)%MOD*comb(nsum,i)%MOD);
                add(res,1LL*comb(n,i)*comb(i,j)%MOD*dp[i][nsum]%MOD);
            } 
            else 
            {
                add(res,1LL*comb(n,i)*comb(i,j)%MOD*comb(nsum,i)%MOD);
                dec(res,1LL*comb(n,i)*comb(i,j)%MOD*dp[i][nsum]%MOD);
            }
        }
    }
    res=1LL*res*(p-1)%MOD;
