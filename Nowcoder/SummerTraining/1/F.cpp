/*************************************************************************
    ...
    > Created Time: 2018-07-19 13:44:38
 ************************************************************************/
...
int main()
{
    ...
    {
        ll ans=1;
        for(ll i=1;i<=n;i++) scanf("%lld",&a[i]),ans=1LL*ans*a[i]%MOD;
        sort(a+1,a+n+1);
        ans=1LL*ans*a[n]%MOD;
        ll now=1,s=1,t=1;
        while(t<n)
        {
            while(a[t]<=now&&t<n) s=1LL*s*a[t]%MOD,t++;
            ll nxt=a[t]-1,k=n-t+1;
            m[0]=0;
            for(ll i=1;i<=n;i++) m[i]=(m[i-1]+pow_mod(i,k))%MOD;
            ll p=(polysum::calcn(n,m,nxt)-polysum::calcn(n,m,now-1)+MOD)%MOD;
            ans=((ans-s*p)%MOD+MOD)%MOD;
            now=a[t];
        }
        printf("%lld\n",ans);
    }
