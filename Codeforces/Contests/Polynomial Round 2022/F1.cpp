...
{
    if(dsum[id]!=0&&tt-cur-1>20) return solve(id,tt-21,rem); 
    if(rem<=dsum[id]) return 1;
    if(dsum[id]==0&&zero>=tt-cur) return p2[tt-cur];
    if(cur>=tt) return 1;
    if(rem>p2[tt-cur-1]*dsum[id]+deq[cur]) 
    {
        int res=p2[tt-cur-1];
        add(res,solve(id,cur+1,rem-p2[tt-cur-1]*dsum[id]-deq[cur]));
        return res;
    }
...
signed main()
{
    ...
    for(int i=n;i>=1;i--)
    {
        ...
        else if(type[i]==2)
        {
            for(int j=ts;j<tt;j++) 
            {
                deq[j]+=1LL*x[i]*p2[tt-j-1];
                if(deq[j]>INF) deq[j]=INF;
            }
            tot+=x[i];
            while(ts<tt&&deq[ts]>=200000) ts++;
            zero=0;
        }
        else if(x[i]>tot) 
        {
