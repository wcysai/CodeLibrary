...
namespace poly
{
    ...
    void solve(int k,int l,int r,vector<int> &p,vector<int> &a,vector<int> &ret)
    {
        if(l==r)
        { 
            ret.resize(1); 
        ...
        solve(k*2,l,mid,lp,a,lret); solve(k*2+1,mid+1,r,rp,a,rret);
        ret.clear();
        ret.insert(ret.begin(),rret.begin(),rret.end()); ret.insert(ret.begin(),lret.begin(),lret.end());
    }
    void multipoint_eval(vector<int> &p,vector<int> &a,vector<int> &ret)
    {
        int n=(int)a.size(); segtree.resize(4*n+1);
        build_tree(1,0,n-1,a);
        solve(1,0,n-1,p,a,ret);
    }
}
int n,m,k;
vector<int> f,g,a,b;
int main()
{
    ...
    poly::multipoint_eval(f,a,b);
    for(int i=0;i<(int)b.size();i++) printf("%d\n",b[i]);
    return 0;
}
