...
namespace poly
{
    ...
    vector<int> fa,fb,fc,fd,fe,ff,fg;
...
{
    Inv[1]=1;
    for(int i=2;i<=399999;i++) Inv[i]=dec(MOD,1LL*Inv[MOD%i]*(MOD/i)%MOD);
}
string str;
int n,m,k;
vector<int> f,g,a,b;
int main()
{
    ...
    for(int i=0;i<(int)str.size();i++) 
        m=(10LL*m+(str[i]-'0'))%MOD;
    for(int i=0;i<n;i++) scanf("%d",&f[i]);
    vector<int> lnf;
    poly::get_ln(f,n,lnf);
    for(int i=0;i<n;i++) lnf[i]=1LL*lnf[i]*m%MOD;
    poly::get_exp(lnf,n,f);
