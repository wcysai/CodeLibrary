...
int main()
{
    ...
    for(int i=1;i<=500000;i++) inv[i]=1LL*invf[i]*fact[i-1]%MOD;
    scanf("%d",&n);
    f.resize(n+1);
    f[0]=1;
    for(int i=1;i<=n;i++) f[i]=(MOD-1LL*pow_mod(i,i-1)*fact[i]%MOD)%MOD;
    for(int i=0;i<=n;i++) printf("%d ",f[i]);
    vector<int> tmp;
    poly::get_inv(f,n+1,tmp);
    for(int i=0;i<=n;i++) printf("%d ",tmp[i]);
    puts("");
    vector<int> tmp2,ff;
    poly::get_ln(tmp,n+1,tmp2);
    for(int i=0;i<=n;i++) printf("%d ",tmp2[i]);
    ff=fft::multiply_mod(tmp2,tmp,MOD);
    for(int i=0;i<n;i++) ff[i]=1LL*ff[i]*invf[i]%MOD;
    for(int i=0;i<n;i++) printf("%d ",ff[i]);
    puts("");
    /*
    g=1LL*g*n%MOD;
    int x=(n+MOD-1LL*f*pow_mod(inv[n],n)%MOD)%MOD;
    int y=(n+MOD-1LL*f*pow_mod(inv[n],n)%MOD+1LL*g*pow_mod(inv[n],n)%MOD+MOD)%MOD;
    printf("%d %d %d\n",0,x,y);
    */
    return 0;
}
