...
int main()
{
    ...
    while(T--)
    {
        ...
        for(int i=1;i<=n;i++)
        {
            ...
            if(i<=(n+1)/2)
            {
                node p=seg.query0(1,1,n,1,pos[i]-1);
                node q=seg.query0(1,1,n,pos[i]+1,n);
                P res;
            ...
            {
                node p=seg.query1(1,1,n,1,pos[i]-1);
                node q=seg.query1(1,1,n,pos[i]+1,n);
                P res;
                if(p==id) res=q.ans[0]; else if(q==id) res=p.ans[0];
                else res=P(p.ans[0].F+q.ans[0].F,0);
                if(res.F>=dif) ans[pos[i]]=1; else ans[pos[i]]=0;
            }
            seg.update(1,1,n,pos[i]);
        }
