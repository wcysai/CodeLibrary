...
struct frac
{
    bigint p,q;
    frac(){}
    frac(int _p,int _q) {p=bigint(_p); q=bigint(_q);}
    frac(bigint _p,bigint _q) {p=_p; q=_q;} 
    void pton()
    {
        bigint g=gcd(p,q);
        p/=g; q/=g;
...
int main()
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        for(int j=0;j<m;j++) cnt[str[i][j]-'A']++;
        for(int j=0;j<26;j++) prob[i][j]=frac(cnt[j],m);
    }
    frac ans=frac(0,1);
    for(int i=0;i<c;i++)
    {
        frac res=frac(val[i],1);
        for(int j=0;j<n;j++)
        {
