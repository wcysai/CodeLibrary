...
void solve(int v)
{
    ...
    if(SZ==0) return;
    vector<ll> ans; ans.clear(); ans.resize(SZ);
    for(auto to:G[v])
    {
        if(centroid[to]) continue;
        for(int i=0;i<(int)save[to].size();i++) {cnt[i]+=save[to][i]; ans[i]+=save[to][i];}
    }
    ans=fft::multiply(ans,ans);
    for(auto to:G[v])
    {
        if(centroid[to]) continue;
        vector<ll> tmp=fft::multiply(save[to],save[to]);
        for(int i=0;i<(int)tmp.size();i++) ans[i]-=tmp[i];
    }
    for(int i=0;i<(int)ans.size();i++) {assert(ans[i]%2==0); cnt[i]+=ans[i]/2;}
