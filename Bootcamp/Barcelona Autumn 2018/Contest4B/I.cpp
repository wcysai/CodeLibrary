...
vector<int> work(int l,int r)
{
    if(l==r) return a[l];
    int mid=(l+r)/2;
    return fft::multiply_mod(work(l,mid),work(mid+1,r),MOD);
}
int main()
{
    scanf("%d",&n);
    ...
    for(int i=1;i<=n;i++)
    {
        if(num[i]) has.push_back(i);
        while(num[i])
        {
            num[i]--;
            ++tot;
            a[tot].resize(i+1);
            for(int j=0;j<=i;j++) a[tot][j]=1;
        }
    }
    ret=work(1,tot);
    ret.resize(n+2);
    ...
    for(int i=0;i<(int)has.size();i++)
    {
        ...
        {
            ans[i][j]+=ret[j];if(ans[i][j]>=MOD) ans[i][j]-=MOD;
            if(j+v+2<=n+1)
            {
                ans[i][j+v+2]-=ret[j];
                if(ans[i][j+v+2]<0) ans[i][j+v+2]+=MOD;
            }
        }
        vector<int> dummy(ans[i]);
        //for(auto it:dummy) printf("%d ",it);
        //puts("");
        for(int j=0;j<=n+1;j++)
        {
            ans[i][j]=dummy[j];
            ...
            {
                dummy[j+v+1]+=ans[i][j];
                if(dummy[j+v+1]>=MOD) dummy[j+v+1]-=MOD;
            }
        }
    ...
    for(int i=0;i<q;i++)
    {
        int v,k;
        scanf("%d%d",&v,&k);
        int id=lower_bound(has.begin(),has.end(),cnt[v])-has.begin();
        if((int)ans[id].size()<k+1) puts("0"); else printf("%d\n",ans[id][k]);
    }
    return 0;
}
