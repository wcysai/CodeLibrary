...
bool point_on_plane(Point &pt,Plane &p)
{
    ...
    return sgn(A*pt.x+B*pt.y+C*pt.z+D,0)==0;
}
...
int main()
{
    ...
    {
        Line l(a[0],a[1]);l.pton();
        Plane pl;
        bool has_plane=false,ans=true;
        for(int i=2;i<n;i++)
        {
            if(!has_plane)
            {
                Line nl(a[0],a[i]);nl.pton();
                if(sgn((l/nl).length(),0)==0) continue;
                else
                {
                    pl=Plane(a[0],a[1],a[i]);
                    pl.pton();
                    has_plane=true;
                }
            }
            ...
            {
                Line ml(a[0],a[i]);ml.pton();
                if(sgn(ml*pl.norm,0)!=0) ans=false;
            }
        }
