/*************************************************************************
    ...
    > Created Time: 2018-11-09 11:10:35
 ************************************************************************/
...
int main()
{
    ...
    vector<int> f(1);f[0]=1;
    for(int i=1;i<=n;i++)
    {
        if(!cnt[i]) continue;
        vector<int> a(cnt[i]+1);
        a[0]=0;
        for(int j=1;j<=cnt[i];j++) a[j]=1LL*invf[j]*comb(cnt[i]-1,j-1)%MOD;
        f=fft::multiply_mod(f,a,MOD);
